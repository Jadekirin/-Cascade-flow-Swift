//
//  NetWaterLayout.swift
//  Swift--瀑布流
//
//  Created by maweilong-PC on 2017/8/2.
//  Copyright © 2017年 maweilong. All rights reserved.
//

import UIKit

protocol NetWaterLayoutDataSource:class{
...
class NetWaterLayout: UICollectionViewFlowLayout {
    //MARK: 对外提供属性
    weak var dataSource : NetWaterLayoutDataSource?
    //MARK: 私有属性
    ...
    fileprivate lazy var colHeights: [CGFloat] = {
        //cols 列数
        let cols = self.dataSource?.numberOfColsInNetWaterfallLayout(self) ?? 2
...
extension NetWaterLayout{
    ...
    override func prepare() {
        ...
        //1 获取列数
        let cols = dataSource?.numberOfColsInNetWaterfallLayout(self) ?? 2
        ...
        for i in startIndex ..< itemCount! {
            ...
            guard let height = dataSource?.NetwaterfallLayout(self, indexpath: indexPath,ItemW: itemW) else {
                fatalError("请设置数据源")
            }
